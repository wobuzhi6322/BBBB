# DEPLOY-RELAY-P0 — 독립 릴레이 사이트 배포 런북

대상: `relay-site/main` 브랜치 — **시청자 후원 메시지 릴레이 독립 사이트** (Vercel + Supabase).
이 브랜치는 통합 `release/relay-p0`에서 프로그램 사이트 파일(admin·releases·다운로드·
프로그램 랜딩·site.js)을 전부 들어낸 것이다(커밋 ec40deb). **새 Vercel 프로젝트**에
배포하며, 기존 gaeideuk.com 프로그램 사이트 프로젝트와는 완전히 분리된다.
`.vercel/` 링크 디렉터리는 의도적으로 제거돼 있다 — 기존 프로젝트로 잘못 배포되는 것을
막기 위함이니 되살리지 말 것.
...
Supabase는 **프로그램 사이트와 같은 기존 프로젝트를 공유**한다(`supabase/schema.sql`
— 라이선스·계정 스키마 — 는 이미 그 프로젝트에 적용돼 있음). 새 Supabase 프로젝트를
만들지 않는다.
...
2. `supabase/migrations-relay-p0.sql` 파일 전체를 붙여넣고 **1회 실행**.
   ...
   - 기존 테이블 데이터 변경 없음(신규 생성 + `bbbb_donation_messages.ip_hash`
     컬럼 추가 1건뿐 — `add column if not exists`, 무손실).
...
⚠️ **이 브랜치를 기존 gaeideuk.com 프로젝트에 배포하지 않는다. 절대 금지.**
(기존 프로젝트는 프로그램 사이트 전용 — 이 브랜치가 올라가면 admin·다운로드가 사라진다.)

1. Vercel 대시보드 → **Add New → Project** → GitHub 저장소 `wobuzhi6322/BBBB` import.
2. 프로젝트 설정:
   - **Production Branch**: `relay-site/main`
...
새 프로젝트 → Settings → Environment Variables, Production 대상. 아래 이름은 전부
현재 `api/` 코드의 `process.env` 사용처에서 수확한 것이다.
...
- [ ] `SUPABASE_ANON_KEY` — 브라우저용 공개 키(`api/site-config.ts`가 클라이언트에
      ...
      `PUBLIC_SUPABASE_ANON_KEY` — 새 프로젝트에는 정본 이름 `SUPABASE_ANON_KEY`
      하나만 설정한다
- [ ] `BBBB_OWNER_EMAILS` — 오너 이메일 콤마 목록(`api/_owner.ts`, 로그인 라이선스
      게이트 예외)
...
**구 배포에서 쓰던 변수 중 이 프로젝트에 넣지 않는 것** (삭제된 프로그램 사이트
API 전용이라 잔존 `api/` 코드에 사용처가 없음 — `npm run check`가 사용처 0건을 확인):

- `BBBB_SHARED_ADMIN_TOKEN` — admin·shared-code API 삭제됨
- `SUPABASE_STORAGE_BUCKET` — 공유 미디어 업로드 API 삭제됨(썸네일 버킷
  `bbbb-web-thumbs`는 코드에 하드코딩)
- `GITHUB_TOKEN` — 릴리스 다운로드 프록시 삭제됨
...
2. 데스크톱 프로그램 관리 화면 → **관리자 릴레이 카드**에서
   **연결 코드와 함께 새 사이트 URL**을 입력한다.
3. 이후 프로그램의 pending 폴링·매칭 보고가 새 사이트로 향한다.

## 6. 배포 후 스모크

기획서 `docs/VIEWER_MESSAGE_RELAY_PLAN.md` §11 그대로 + 독립 사이트 선행 3항.
...
**DB(Supabase)**: PART 1(웹 플랫폼 스키마)은 통합 브랜치 시절부터 공유되는
...
drop 대신 웹 롤백(프로젝트 삭제/직전 배포 promote)만. PART 1까지 포함한 전체
드롭 목록은 `supabase/migrations-relay-p0.sql` 상단 롤백 노트에 있다(운영 데이터가
없는 초기 상태에서만 사용).
