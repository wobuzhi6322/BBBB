...
async function getRedeemableCode(code: string, supabase: ReturnType<typeof serviceClient>): Promise<LicenseCodeRow> {
  let result = await supabase.from(licenseCodesTable).select(codeSelect).eq("code_hash", hashCode(code)).single();
  if (isMissingFeatureFlagsColumn(result.error)) {
    result = await supabase.from(licenseCodesTable).select(codeSelectWithoutFeatures).eq("code_hash", hashCode(code)).single();
  }
  if (result.error || !result.data) {
    throw new Error("코드를 찾을 수 없습니다.");
  }

  const licenseCode = normalizeLicenseCode(result.data as LicenseCodeRow);
...
async function getExistingActiveLicense(
  userId: string,
  supabase: ReturnType<typeof serviceClient>
): Promise<{ data: LicenseRow | null; error: null }> {
  ...
  return { data: result.data ? normalizeLicense(result.data as LicenseRow) : null, error: null };
}

async function insertAccountLicense(payload: Record<string, unknown>, supabase: ReturnType<typeof serviceClient>): Promise<LicenseRow> {
  ...
  return normalizeLicense(result.data as LicenseRow);
}

async function updateAccountLicense(
  licenseId: string,
...
function normalizeLicense(license: LicenseRow): LicenseRow {
  return { ...license, feature_flags: normalizeFeatureFlagsWithNotes(license.feature_flags, license.notes) };
}

function normalizeLicenseCode(code: LicenseCodeRow): LicenseCodeRow {
  return { ...code, feature_flags: normalizeFeatureFlagsWithNotes(code.feature_flags, code.notes) };
}
