...
헤더는 로고, 중앙 메뉴, 우측 액션으로 구성한다. 헤더 아래에는 투네이션식 카테고리 바를 두어 시그니처, 미디어, 벽지, 랭킹, 공유 코드, OBS 연결을 빠르게 스캔하게 한다. 본문은 히어로, 상태, 카테고리, 기능, 작동 방식, 업데이트, 다운로드, 설치, 계정, 푸터 순서다. 작동 방식, 업데이트, 설치 설명은 반복 카드가 아니라 선형 워크플로우, 구분선 타임라인, 체크리스트형 설명으로 만든다.

## Components

카드는 기능 목록과 계정/다운로드처럼 실제로 묶음이 필요한 곳에만 사용한다. 페이지 섹션 자체를 카드처럼 띄우지 않고, 설명 섹션은 여백, 세로 라인, 행 구분선, 타이포그래피 계층으로 만든다. 버튼과 카드 모서리는 8px를 기본으로 한다. 큰 장식용 CSS gradient, blob, orb, SVG 배경은 사용하지 않고 실제 이미지 자산이나 단색 표면으로 깊이를 만든다.
